...
| D18 | arXiv 查询构造(M31) | 废整句短语 `all:"<原文>"`,改官方手册 §5.1 模型:分词 → 清洗 → `all:t1 AND all:t2`;字段前缀(ti:/au:/abs:/cat:…)与大写布尔(AND/OR/ANDNOT)直通;`buildSearchQuery` 纯函数单测 | 对 export.arxiv.org 实证:整句短语 `all:"diffusion transformer"` 仅 1570 命中,官方姿势 AND 为 7486(短语化大量漏检);不配对引号时 API **不报错**,把查询静默改写成 `all:a OR all:b OR …` 返回跑偏结果——所以清洗必须在客户端做(TermStrip 剥引号/括号/冒号/反斜杠,连字符与点实证安全保留);首尾/重复操作符丢弃,字段前缀先于清洗匹配 | 继续整句短语(漏检+静默畸变,已证伪);不清洗直通原文(特殊字符触发静默改写);客户端自行打分排序(数据量不够,违背 API relevance) |
| D19 | Agent 推荐链路易用性(M32) | search_articles 并发抓取(detached fetch + 硬超时:单源 12s/总 20s/并发 4);卡片解析加固(```json 围栏/未闭合/尾逗号修复/尾文保留)+ 流式折叠占位;AnswerAccumulator 只留最后一轮文本进气泡;系统提示明示 tool_calls 通道 | 用户实测两症状:① 推荐时气泡直接显示 JSON——三层根因:流式期间 parseCards 不执行(原文逐 token 打出)、解析兜底太脆(围栏不闭合/尾逗号即全灭)、AgentRunner 每轮 delta 全拼进最终答案(中间轮的计划碎语/调用 JSON 混入气泡并入库);② 工具调用缓慢——SearchArticlesTool 串行循环 19 源 × 1-3s(超时源 45s),一次调用分钟级。关键实现决策:OkHttp execute() 是阻塞调用、协程取消打不动,单源硬超时必须 detached async + await 竞速(放弃而非取消),孤儿调用由 OkHttp 自身超时收尸 | 保持串行只调小超时(慢源叠加仍分钟级);withTimeoutOrNull 直接包阻塞调用(取消不生效,伪超时);流式期间整块隐藏(用户看不到正文渐入,体验倒退);中间轮文本直接丢弃不留 fallback(模型调完工具不答时气泡全空) |
| D20 | 论文记忆:大纲索引+分节钻取+文内检索(M33) | 打开论文从重排 EPUB 建 PaperIndex(ar5iv ltx_section 分节,无结构退化 4000 字符块);系统提示内联 摘要+大纲 取代截断全文;三工具 get_paper_outline/read_paper_section(offset 续读)/search_in_paper(段落级)取代 get_article_text;TOOL_CONTENT_MAX_CHARS 2000→4000 | 用户实测"AI 读论文会截断":旧链路先把 EPUB 用 Jsoup .text() 拍平(ar5iv 章节结构丢光)再砍两刀——系统提示内联 6000、工具返回 6000,且 AgentRunner 对工具结果还有第三刀 2000;典型论文 3-8 万字符,智能体只见前 10-20%。选型:"长文外置、工具即记忆存取原语"(人类式阅读:先看目录再钻章节)胜过向量 RAG(端侧 embedding 50-200MB+索引延迟,单文档 QA 增益甚微)、分页滑窗(盲目翻页烧轮次)、分节预摘要(每次打开烧 token,列为后期可选增强) | 调大截断上限(6 万字符塞 context,贵且稀释注意力);端侧向量 RAG(过度工程);read 无 offset 续读(长节读不完);保留 get_article_text 并存(模型会偷懒调旧工具拿截断文本,与 paperMode 提示冲突) |
...
> **M31(arXiv 查询构造,D18)已验收(2026-08-04,export.arxiv.org 实测 API 探针 + 221 单测;真机冒烟待设备回连补做)**:废整句短语检索,改官方手册 §5.1 模型。取证:① `all:"diffusion transformer"`(旧行为)1570 命中 vs `all:diffusion AND all:transformer`(官方)7486——短语化是漏检主因;② 不配对引号被 API 静默改写成 `all:a OR all:b OR …`(返回 feed 的 `<title>` echo 实证,官方手册亦建议看 title 校验),不报错;③ `all:GPT-4` 连字符 4715 命中且 echo 原样——保留;通配符不可靠(419)——剥除。实现:`buildSearchQuery` 纯函数:分词 → 字段前缀(ti:/au:/abs:/co:/jr:/cat:/rn:/all:)先于清洗匹配直通 → TERM_STRIP 剥引号/括号/内嵌冒号/反斜杠(保字母数字 `._-`)→ 普通词补 `all:` 前缀,默认 AND 连接,大写布尔直通,首尾/重复操作符丢弃;`searchAndCategory` 关键词部分加括号防用户 OR 泄漏出分类过滤;空查询零请求直接返回空。Agent 工具描述补字段语法说明。**221 个 JVM 单测绿**(新增 buildSearchQuery 专项 6 例;首版实现被测试抓出两个真 bug——清洗先于前缀匹配致 `ti:` 被剥、尾随操作符未丢弃,已修)。

> **M34(双语翻译修复+提速,2026-08-07,OnePlus 真机 adb 全程实证)**:双语翻译功能(外部 WIP 并入)三连修。**① PDF 白屏**:重构成 `loadPdf()` 后把调用放在了 `webView = this` 赋值之前,`webView?.loadUrl` 空安全调用静默吞掉首次加载——不报错不崩溃;插桩取证(renderer 活着、loadPdf 打了、拦截零调用),赋值前移一行修复。**② 译文不渲染(双 bug)**:(a) Kotlin 把 `JSONArray` 字符串内联进 `evaluateJavascript("window.__applyTranslations($arr)")`,JS 收到的是**数组字面量**而非 JSON 字符串,`JSON.parse` 收到 "[object Object]" 静默 catch 返回——译文从未落 DOM;bilingual.js 改为字符串/数组双收。(b) 段落分块(50/块)上报,第一块就启动翻译任务,`translateJob.isActive` 把后续 220 段全部丢弃;bilingual.js 增加 `onParagraphsDone(total)` 完成标记,Kotlin 加 `paragraphsComplete` 闸门,段落集齐才启动。实证:修复前 270 段只应用 50,修复后 270 段一次全量下发。**③ 提速四件套**:翻译请求显式 `thinking: {"type":"disabled"}`(DS-v4 思考模式默认开,每批白交 10-60s 推理税——龟速主因,官方请求级开关,`streamChat` 加 `disableThinking` 参数只翻译路径用);批次串行改 4 路 worker 池(Channel 队列,完成即写 Room、按完成顺序推进度);批次 2000→8000 字符(38 批→~10 批);进度条加流式字数(「翻译中 3/10 · 1847 字」)。实测 AV-AIVAT 论文 ~250 段:99 秒完成(旧版估算 25 分钟+,约 15×)。**264 个 JVM 单测绿**(TranslateBatcher 两例改为上限相对化,OpenAICompatProvider 新增 thinking 开关双向断言)。已知残留:译文块内 $...$ 公式显示为原始 LaTeX 文本(不渲染 MathML);翻译代码仍未提交(外部 WIP,提交时机用户定)。

> **M35(PDF 缩放钳制,2026-08-07,用户手测确认)**:论文 PDF 完全缩小后内容堆左半——页面本为 `margin:auto` 居中排版,但 viewer viewport 元标签无 `minimum-scale`,捏合缩小跌破 scale=1 后 Chromium 把内容锚在左上、右半留白。修复:`minimum-scale=1`,缩小钳制在整页适配宽度(最小可读状态);放大路径不受影响。viewer.html 一行改动。

> **M36(译文公式渲染,2026-08-07,OnePlus 真机截图实证 tmp/m36-13.png)**:译文块 $...$ 原文直出(用户报"一些公式渲染不出来")。方案:不引 KaTeX/MathJax 依赖,`__applyTranslations` 落译文后按 **alttext 匹配**(空白归一)把 $...$ 片段替换为原文同段 `<math>` 元素的克隆——模型被提示词约束逐字保留公式,命中率极高;未命中(货币 $0.07、模型自造公式)保持原文,不比修复前差。真机实证:$r_i^+$/$z_i^b$/$b\in\mathcal{B}=\{mis,clean,cor,irr\}$ 等复杂公式在译文里渲染为真数学。独立展示公式块(ltx_equation)本就不进翻译集,保持原文——符合设计。

> **M37(词卡面板跟随阅读主题,2026-08-07,用户手测确认)**:EPUB 阅读器内查词面板的颜色全部来自 `hp_*` 资源色,只随**系统**夜间模式(values-night)翻转——系统亮色 + 阅读器设夜间时,暗页面上弹出一张刺眼白纸面板(在案遗留项)。修复:新增 `WordCardPalette`(WordCardPanel.kt,纯 Int 字面量,JVM 可测)按**阅读器主题**供色——dark 复用设计系统夜间值(#171512 纸 = M27 夜读页底色,面板与页面融为一体、2dp 墨色顶规则线分界,延续 M16「无边浮影」语言;#ECE7DB 墨 / #A8A29A 次级 / #6F6A62 弱),sepia 派生自 Readium 羊皮纸页(#FAF4E8 / #5F4B32),light 镜像日间 hp_* 不变;`applyTheme(themeName)` 程序化重建容器 LayerDrawable(纸底+顶规则线)、全部文字色、关闭钮 tint、填充/描边双钮(描边钮 1dp ink 笔触,填充钮 enabled=ink/disabled=faint 并入 setSaved)。接线三处:面板创建时、installNavigatorFactory 读档后、applyReaderSettings 主题翻转时(面板开着也实时换肤)。PaperReaderActivity 无阅读主题概念,面板映射系统夜间(与该屏 Compose chrome 的 values-night 行为一致)。布局改动仅分隔线补 id。**269 个 JVM 单测绿**(新增 WordCardPaletteTest 5:三主题色值/未知回退 light/主题互异)。

> **M38(主界面换色主题,2026-08-07,预览页选型用户拍板「全上」)**:主界面(四 tab + 设置 + Agent + 阅读器 chrome)开放六款双色主题,设置页可切换。**边界**:阅读器版面(light/sepia/dark 阅读主题、词卡面板 M37、高亮)分文不动;功能红(代理徽章/错误)在所有主题下不变。**设计**:每主题 = 深主色 + 同族浅底容器色的双色对,铺在微染色的纸/墨中性底上——色彩只住强调位(选中 tab 花体字+下划线、填充按钮、开关、徽章),版式保持编辑中性;六款 = 经典墨(现状,单色:accent 即墨色)+ 森林绿/勃艮第红/普鲁士蓝/鸢尾花/燕麦拿铁;每款带暗色变体(主色提亮一档,深底可读)。预览先行:design-previews/2026-08-07-app-themes(主界面实样机 × 6,亮/暗一键全翻,Pinyon 花体底栏实证选中态吃主题色)。**实现**:`AppTheme` 枚举(ui/AppTheme.kt,纯 Long 色值 → JVM 可测;fromId 未知回退 classic)+ `AppThemeController`(SharedPreferences "app_prefs" + StateFlow,Application.onCreate 初始化);`Theme.kt` 固定双方案改为 `schemeFor(palette, dark)` 参数化构建(classic 逐色复刻旧方案,仅 inversePrimary 这一 app 内无用角色改挂 accentSoft);`HandypageTheme(appTheme=…)` 四调用点(MainActivity/SettingsActivity/PaperReaderActivity 随系统明暗,ReaderActivity 明暗仍由阅读主题驱动 M26)collectAsState 实时换肤;EditorialNavBar/EditorialTabRow 选中内容色 onSurface → primary(classic 下两者相同,零视觉差)。设置 hub 新增「界面主题」行(实时摘要 主题名 + EN)→ AppearanceSettingsPage(双色圆点 + 中英文名 + 选中对勾,点按即换不重创)。**275 个 JVM 单测绿**(新增 AppThemeTest 6:id 双向解析/未知回退/classic 逐色复刻/六主题强调色互异/暗色主色提亮/实色全不透明)。编译期揪出一个真 bug:`palette(dark)` 形参遮蔽同名属性,`if (dark) dark else light` 返回了 Boolean,改 `this.dark`。**真机验收(2026-08-07,adb 截图 tmp/m38-1-settings-hub.png + 用户手测确认)**:设置 hub「界面主题」行在列、摘要实时(经典墨 · Classic Ink),主题切换与全局重绘用户手测通过。

> **M39(论文工具调用上限放宽,2026-08-07)**:用户实测"AI 分析论文时工具调用次数容易用尽导致终止"。根因:AgentRunner 的硬上限 `MAX_TOOL_ROUNDS = 5` 是 M5 为新闻聊天定的,而 M33 论文记忆工具刻意细粒度——`read_paper_section` 按 4000 字符窗口分页,续读一页就烧一轮 provider round,认真读一篇论文 = 大纲 1 轮 + 三四节带续读 8-10 轮,5 轮必然腰斩。修复:上限参数化(`maxToolRounds` 构造参数,默认 5 不变),新增 `PAPER_MAX_TOOL_ROUNDS = 12`(大纲 + ~4 节带续读的余量),ChatController 按 `paperIndexProvider != null` 分档传入;日 token 预算仍是真正的成本闸门(每轮前置检查),轮次上限只防失控循环。新闻聊天路径零改动。**276 个 JVM 单测绿**(新增:论文档上限下持续工具调用跑满 12 轮才 ToolLimitReached)。

> **M40(生词本管理:分组/搜索/掌握度/批量操作/CSV 导出,2026-08-08)**:生词本从"平铺新词列表"升级为可管理的词书。同一单词从不同文章保存会产生多行((word, articleUrl) 唯一索引),现在按 lemma 分组(无 lemma 回落表面形,大小写归一)——一组一行,×N 角标计数,展示字段取最新行(音标/释义/定义为空时回落最新非空值),掌握度取全组最小值( least-mastered wins)。**DB v7→v8**:`vocab_words` 加 `mastery` 列(0 新词/1 学习中/2 已掌握,ALTER TABLE DEFAULT 0,存量全部为新词);DAO 新增 `updateMastery(ids, mastery)` / `deleteByIds(ids)` 批量操作。**掌握度生效点**:行首 8dp 圆点三色(primary=新词/tertiary=学习中/outline=已掌握);详情弹窗内掌握度按钮循环 0→1→2→0 并落库;**已掌握单词停止文内弱高亮**(ReaderFragment 走 `highlightTerms()` SQL 加 `mastery < 2`,PaperReaderActivity 在 observeAll().map 里过滤)。**头部工具条**(内容区顶部,scaffold-free 不变):搜索框(词头/释义/定义大小写不敏感 contains)、掌握度 FilterChip 四枚(全部/新词/学习中/已掌握)、排序 DropdownMenu(最近/频次/字母,当前项打勾)、导出按钮——CSV 经 SAF `CreateDocument("text/csv")` 全量备份(永远导出全部行而非过滤视图),RFC-4180 转义,Toast「已导出」。**选择模式**:长按入行,头部换为 关闭/已选 N 项/全选/标为已掌握/删除(均带确认),选中行 surfaceVariant 底色,批量操作作用于组内全部行 id 后退出。详情弹窗改按组 key 持有状态(每次重组从最新派生列表查回,掌握度编辑实时反映;组被删则自动关闭),正文列全部出处(例句 + 来源·日期,hairline 分隔),删除 = 删除该组全部记录(确认文案带词头+条数)。分组/过滤/排序/CSV 全部纯 JVM(`VocabGrouping.kt` / `VocabExport.kt`)。**290 个 JVM 单测绿**(新增 VocabGroupingTest 10 + VocabExportTest 4)。

> **M32(Agent 推荐链路易用性,D19)已验收(2026-08-04,233 单测含 8 源并发计时/挂起源放弃/总预算三例;真机手测待设备回连)**:四连修。A 缓慢主因:SearchArticlesTool 改并发(4 路分块)+ 单源 12s/总 20s 硬超时——阻塞 OkHttp 调用协程取消打不动,抓取跑 DETACHED_FETCH(SupervisorJob+IO),`withTimeoutOrNull` 竞速 await,挂起源放弃并在结果附注"N 个源响应超时未计入";8 源×300ms 测试 2400ms→636ms。B 误传 JSON:parseCards 兼容 ```json 围栏/未闭合围栏(截断流仍尝试解析)/尾逗号修复/围栏后尾文并入正文,整体失败保持旧语义(只显示正文,永不裸 JSON);流式期间 `splitStreamingCards` 把在建卡片块折叠为「正在生成推荐卡片…」占位(新字符串 agent_cards_generating)。C 中间轮碎语:新 `AnswerAccumulator`(agent 包,纯 JVM),ToolStarted 时把当轮文本降级为 fallback,只有最后一轮文本进气泡与 Room。D 系统提示明示"调用工具必须走 tool_calls 通道,正文禁止输出工具名/参数/调用 JSON"。**233 个 JVM 单测绿**(新增 AnswerAccumulator 5 + parseCards 加固 5 + SearchArticles 并发/超时 3;旧"未闭合返回 null"用例按新语义反转)。待办:模型把工具调用写成正文 JSON 的候选根因尚无实锤,需真机 logcat(HandypageAI tag)验证,D 项提示词为先行缓解。

> **M33(论文记忆:大纲索引+分节钻取+文内检索,D20)已验收(2026-08-04,246 单测;真机手测待设备回连)**:根治"AI 读论文截断"。根因链比上报时更糟:EPUB→Jsoup .text() 拍平丢结构 → 系统提示/工具各砍 6000 → **AgentRunner 对工具结果还有第三刀 2000**(TOOL_CONTENT_MAX_CHARS),智能体实际只见论文前 2000 字符。实现:新 `PaperIndex`(agent 包,纯 JVM+Jsoup):ar5iv `div.ltx_abstract`/`section.ltx_section`/`ltx_bibliography` 分节(子节折入父节),无结构文档退化 4000 字符 Part 块;节文本保段落界("\n\n" 连接)供段落级检索;`readSection` 4000 窗口+offset 续读提示,`search` 命中段带节标签。接线:ChatController 新增 `paperIndexProvider`(null 不缓存,转换完成前工具报"论文内容尚未就绪"),paper 模式下系统提示内联 摘要+大纲、三工具(get_paper_outline/read_paper_section/search_in_paper)**取代** get_article_text,Prompts.agentSystem 加 paperMode 引导("不要凭摘要臆测未读章节");TOOL_CONTENT_MAX_CHARS 2000→4000 与阅读窗口对齐(历史总预算 8000 不变);PaperReaderActivity 按 zip 配方建索引(保结构标记)。新闻文章路径零改动。**246 个 JVM 单测绿**(新增 PaperIndex 8 + PaperTools 4 + Prompts paperMode 1)。已知边界:MathML 公式经 .text() 是扁平文本(与旧行为一致);参考文献节参与检索(噪声可接受)。
